...
impl DbManager {
    ...
    pub fn update_history(&self, file_path: &str, unixtime: i64, page_index: i64, page_direction: PageDirection) {
        let page_direction_int = page_direction as i64;
    ...
    pub fn update_page_direction(&self, file_path: &str, page_direction: PageDirection) {
        let page_direction_int = page_direction as i64;
        self.conn.execute("update open_file_history set page_direction = ?1 where location_path = ?2",
                          params![page_direction_int, file_path.to_owned()]).unwrap();
    }
